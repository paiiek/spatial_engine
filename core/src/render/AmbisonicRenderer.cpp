...
void AmbisonicRenderer::prepareToPlay(const geometry::SpeakerLayout& layout,
                                       double /*sample_rate*/)
{
    layout_ = layout; // cache for applyPendingDecoderTypeChange()
    decoder_.prepare(layout);
    applied_decoder_type_ = decoder_type_.load(std::memory_order_relaxed);
...
void AmbisonicRenderer::setDecoderType(int type) noexcept {
    if (type < 0 || type > 4) type = 0; // clamp to PINV
    decoder_type_.store(type, std::memory_order_relaxed);
}

void AmbisonicRenderer::applyPendingDecoderTypeChange() {
    // CONTROL THREAD ONLY — allocates (decoder_.prepare() allocates).
    const int pending = decoder_type_.load(std::memory_order_relaxed);
    if (pending == applied_decoder_type_) return;
    decoder_.setDecoderType(static_cast<ambi::DecoderType>(pending));
    decoder_.prepare(layout_);
    applied_decoder_type_ = pending;
}
    ...
    int    num_samples)
{
    ...
    for (int obj = 0; obj < N; ++obj) {
        ...
        if (active_order == 1) {
            ...
            const float w = c.W;
            const float y = c.Y * scale1;
            const float z = c.Z * scale1;
            const float x = c.X * scale1;
            ...
            for (int n = 0; n < num_samples; ++n) {
                const float s = src[n];
                W[n] += s * w; Y[n] += s * y; Z[n] += s * z; X[n] += s * x;
            }
