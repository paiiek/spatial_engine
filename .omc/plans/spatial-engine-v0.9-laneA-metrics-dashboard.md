...
- [x] A-M4 dashboard.html/js + 자체 canvas 차트 — done 2026-05-30. NEW static/js/minichart.js (self-hosted ~210 LOC canvas time-series: axes/grid/per-series line/peak marker, 60s rolling window, zero deps — DD-C LOCKED) + static/js/dashboard.js (`/ws/metrics` connect+exp-backoff reconnect mirroring ws_client.js; 60s ring buffer per series via MiniChart.push; CPU avg/peak [A9-Q8: peak own line, clamped [0,100]], xrun Δ, p99 charts; warning log last 50; latest-wins snapshot on connect; binaural demote flag; reset-demote button DOM placed (behavior=A-M5); window.__dashboard test surface). dashboard.html replaced stub wholesale: §1 engine status bar / §2 CPU·xrun·p99 charts / §3 binaural panel (#btn-reset-demote) / §4 warning log / §5 64-cell object activity grid. NO external CDN (loads only /static/js/{minichart,dashboard}.js). NEW test_dashboard_smoke.py (AC5): headless chromium, reuse uvicorn fixture, assert canvas mount + draw-on-metrics (CanvasRenderingContext2D.prototype spy) + reset button DOM; importorskip+PWError skip guard. Playwright AVAILABLE (chromium-1217 present, smoke green). webgui pytest 58 passed (57 after A-M3 + 1 new smoke), no regression; both JS pass `node --check`.
- [x] A-M5 reset_demote 버튼 e2e — done 2026-05-30. server.py `_dispatch_to_osc`: NEW `elif mtype=="binaural_reset_demote": osc_send_fn("/sys/binaural_reset_demote", 1)` 분기(제어 평면 — `_POSITION_MTYPES` 비포함 → ADR 0013 AI-mode position guard 무관, 전 모드 통과). dashboard.js: NEW 최소 `/ws` control 소켓(ws_client.js exp-backoff idiom 미러, dashboard.html 은 main-UI ws_client.js 미로드) + `onResetDemote()`→`ctrlSend({type:"binaural_reset_demote"})`, reset 버튼 click 배선, init()에서 `ctrlConnect()`, `window.__dashboard` 에 `onResetDemote`/`ctrlSend` 노출. test_dispatch.py: NEW 2 tests(AC6 — `/sys/binaural_reset_demote`,(1) 송신 + AI-mode 통과). test_dashboard_smoke.py(playwright): 버튼 click→`WebSocket.prototype.send` spy 로 `binaural_reset_demote` payload 검증(in-test 추가). 엔진 측 무변경(핸들러 기존재). webgui pytest 60 passed (A-M4 58 + dispatch 2), 무회귀. dashboard.js `node --check` pass.
- [x] A-M6 문서 + README + ADR 확정 — done 2026-05-30. **Lane A COMPLETE (6/6).** docs/TESTING.md §A(대시보드 접근 `/dashboard`·`/ws/metrics`, 6필드 메트릭 의미표, 5개 신규 테스트 실행 절차); ENGINE_OVERVIEW §4.4 #8 "부재"→"기본 대시보드 제공" (1Hz·클램프·sticky flag·정적 그리드 한계 명시, 오버셀 금지); README WebGUI 섹션 §"실시간 메트릭 대시보드(`/dashboard`)" pointer; NEW docs/adr/0020-sys-metrics-channel.md (얇은 채널-doc — `,s` key=value 재사용/신규 인코더 0, 6필드 의미표, steady_clock-wall + 스칼라 P² p99 결정, 드롭 대안 5종) + adr/index.md 등록. markdown only — 내부 링크 무파손, 코드와 일치(MetricsEmit.h 필드셋 검증). pytest ui/webgui/tests/ 60 passed (무회귀).
