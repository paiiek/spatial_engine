...
- **현재 안정 지점**: VAP + VBAP3D + MDAP + WFS + **128 리프트** + **룸 후기리버브 라이브(⑥a/b) + early 코어(⑥c)** + 회귀 0 + WERROR=ON **양 빌드 130/130**. 브랜치 `feat/dreamscape-convergence` **origin 푸시됨(main +26)**.
- **다음 증분(순서)**: **⑥d early reflections 라이브 결선** (RoomEarly 코어 + per-object predelay/absorption-EQ/width-spread VBAP/ring-buffer → SpatialEngine early bus → 실바이너리 스모크) → ⑥e cluster + late opp 소스바이어스/diffuse 블렌드 → ⑦ 디코릴 ⑧ 헤드트래킹/바이노럴 ⑨ ADM ⑩ per-obj EQ.
  - ⑥d 참조: RoomEngine.cpp:371-485(per-obj predelay/EQ/6-image ring-buffer fan-out), 113-165(blendVbapWithUniformDiffuse), kErRingLen=512/kEarlySpreadSamples=3/kErTri3{1,2,1}/4. 결선: per-obj reverb_send 신호+위치 → early 처리 → mix_buf_. ⑥e 참조: 595-647(cluster), 543-583(late opp). **③' (분리·보류)**: ported `computeHorizontalVbap/Horizontal·SpatialMdap` std::vector→고정버퍼 no-alloc(ported-프레임 렌더러 채택 시 필요) — DoD §9 충족용.
- **후속 메모(증분 중 발견)**:
