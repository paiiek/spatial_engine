...
- **⑥ 잔여 = superset 도달 작업(진행중)**: **⑥f** 거리-게인/early·late gain-dB 커브 **이식**(7 param: distanceNear/Far/Linearity, lateGainClose/FarDb, earlyGainClose/FarDb — 현재 포트는 간이 게인공식, 레퍼런스 거리모델 미이식=DSP 충실도 갭) + `/room/distance`·`/room/early/gain`·`/room/late/gain` OSC. **predelay-ms**(1, ring-wrap pds<stride 불변식 보류). **/room/preset recall**(scene 스냅샷 인프라 통합). ⚠ ⑥f시 /room/set이 15→~22 float → MAX_ARGS 16→24 bump 필요.
- **다음 증분(순서)**: ⑥f(거리/게인 커브) → /room/preset → ⑦ 디코릴 ⑧ 헤드트래킹/바이노럴 ⑨ ADM ⑩ per-obj EQ. xlsx 권위 룸계약=05.룸엔진+02.세션상태(SpatialSessionState.h 22 room param), per-param 룸 OSC 표는 xlsx에 **없음**(/room/*=mmhoa 고유).
