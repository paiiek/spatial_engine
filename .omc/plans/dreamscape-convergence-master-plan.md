...
- **⑥ 잔여 = /room/preset (명명 recall, 선택적)**: 22 param은 개별 OSC + /room/set atomic 번들로 완전 노출됨. 잔여는 **명명 프리셋 recall**(workflow 편의)=scene 스냅샷 인프라(SceneSave/Load)에 룸 param 통합 필요. 파라미터 커버리지가 아니라 recall 워크플로우 기능.
- **다음 증분(순서)**: /room/preset(선택) → ⑦ 디코릴 ⑧ 헤드트래킹/바이노럴 ⑨ ADM ⑩ per-obj EQ. xlsx 권위 룸계약=05.룸엔진+02.세션상태(SpatialSessionState.h 22 room param), per-param 룸 OSC 표는 xlsx에 **없음**(/room/*=mmhoa 고유, superset).
