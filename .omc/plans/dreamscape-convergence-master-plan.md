...
- **⑥ 잔여 = ⑥g predelay-ms(→22/22) + /room/preset**: **⑥g** roomEarlyPredelayMs 노출 — renderRoomEarly의 고정 kRoomEarlyPredelayMs→멤버, pds는 이미 er_predelay_max_-1<stride(=max+MAX_BLOCK+8) clamp라 **단일분기 wrap 불변식이 모든 predelay서 성립**(이전 "보류"는 과보수, 안전). `/room/predelay ,f`+set 22→23. **/room/preset**=명명 recall, scene 스냅샷 인프라 통합 필요.
- **다음 증분(순서)**: ⑥g(predelay→22/22) → /room/preset → ⑦ 디코릴 ⑧ 헤드트래킹/바이노럴 ⑨ ADM ⑩ per-obj EQ. xlsx 권위 룸계약=05.룸엔진+02.세션상태(SpatialSessionState.h 22 room param), per-param 룸 OSC 표는 xlsx에 **없음**(/room/*=mmhoa 고유).
