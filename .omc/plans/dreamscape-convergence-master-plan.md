...
- **현재 안정 지점**: VAP+VBAP3D+MDAP+WFS + **128 리프트** + **룸: 후기 FDN(⑥a/b)+early(⑥c/d)+diffuse(⑥e-1) 라이브 + cluster 코어(⑥e-2)+absorption-EQ 비콰드 코어(⑥e-3a) (둘 다 미결선)** + 회귀 0 + WERROR=ON **양 빌드 132/132**. push됨 **main +34**.
- **다음 증분(순서)**: ⑥ 룸 마감 잔여 — **⑥e-3b early predelay(:374-395) ring + per-obj RoomBiquad EQ(:397-406) 결선**(renderRoomEarly에 predelay 라인+per-obj earlyClusterHp/Lp 삽입, ring 탭 전; 기본 코너로 OSC 무관) / **⑥e-4 룸 OSC params** ⚠ **외부 API 설계 결정 필요**(레퍼런스는 `/room/preset/recall` 프리셋 방식—per-param OSC 미러 없음 → mmhoa 고유 주소체계: roomEngineEnabled gate + roomT60/halfExtents/roomEarly*/roomCluster*/EQ 코너 setters→room_fdn_/room_early_params_/room_cluster_/EQ. 사용자 확인 권장) / late opp 소스바이어스(:543-583) / **⑥e-2-wire cluster 결선**(send :414-419 + process :595-647 버스EQ(RoomBiquad)+RoomCluster::process+per-spk diffuseGains; xDel(⑥e-3b)+opp-bias+OSC(⑥e-4) 의존→마지막). → ⑦ 디코릴 ⑧ 헤드트래킹/바이노럴 ⑨ ADM ⑩ per-obj EQ.
