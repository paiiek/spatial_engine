...
- **현재 안정 지점**: VAP+VBAP3D+MDAP+WFS + **128 리프트** + **룸: 후기 FDN(⑥a/b)+early(⑥c/d)+diffuse(⑥e-1) 라이브** + 회귀 0 + WERROR=ON **양 빌드 130/130**. push됨 **main +31**.
- **다음 증분(순서)**: ⑥ 룸 마감 잔여 — **⑥e-2 cluster**(feedforward-tap 확산, :414-419 send + :595-647 process) / **⑥e-3 early predelay(:374-395)+absorption EQ(:397-406)** / **⑥e-4 룸 OSC params**(roomEngineEnabled gate + roomT60/halfExtents/roomEarly*/roomCluster* setters→room_fdn_/room_early_params_) / late opp 소스바이어스(:543-583, 동적 FDN 게인). → ⑦ 디코릴 ⑧ 헤드트래킹/바이노럴 ⑨ ADM ⑩ per-obj EQ.
