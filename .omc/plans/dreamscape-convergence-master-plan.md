...
- **다음 증분**: ② MDAP 이식(VBAPRenderer width 경로를 ported computeHorizontal/SpatialMdap로 교체 또는 신규) + VBAP 3D 5단 고도 레이어링(`fillVbapMaskForObject` 이식) ③ WFS 전모드 ④ Phase 0.5 128 리프트 ⑤ 2D-VBAP/MDAP std::vector→고정버퍼(no-alloc).
