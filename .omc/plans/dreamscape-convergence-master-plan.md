...
- **현재 안정 지점**: VAP+VBAP3D+MDAP+WFS + **128 리프트** + **룸: 후기 FDN(⑥a/b)+early(⑥c/d)+diffuse(⑥e-1)+early predelay/EQ(⑥e-3b) 라이브 + cluster 코어(⑥e-2)+EQ비콰드 코어(⑥e-3a)** + 회귀 0 + WERROR=ON **양 빌드 132/132**. push됨 **main +36**.
- **다음 증분(순서)**: ⑥ 룸 마감 잔여 — **late opp 소스바이어스(:543-583)**(후기 FDN 게인을 정적 cube-corner→소스방향 반대 동적 바이어스, finishBlock; byte-faithful 자율가능) / **⑥e-2-wire cluster 결선**(send :414-419 `clusterBusInput += xdel*cSend(roomClusterSend01·0.48)` + process :595-647 버스EQ(RoomBiquad)+RoomCluster::process+per-spk diffuseGains; xdel(⑥e-3b 완료)+opp-bias+OSC(roomClusterSend01) 의존) / **⑥e-4 룸 OSC params** ⚠ **외부 API 설계 결정 필요**(레퍼런스 `/room/preset/recall` 프리셋만—per-param OSC 미러 없음 → mmhoa 고유 주소체계 설계: roomEngineEnabled gate + T60/halfExtents/roomEarly*/roomCluster*/EQ 코너. cluster-wire/튜닝 unblock. 사용자 확인 권장). → ⑦ 디코릴 ⑧ 헤드트래킹/바이노럴 ⑨ ADM ⑩ per-obj EQ.
