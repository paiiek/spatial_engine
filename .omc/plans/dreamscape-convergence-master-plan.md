...
- **현재 안정 지점**: VAP+VBAP3D+MDAP+WFS + **128 리프트** + **룸: 후기 FDN(⑥a/b)+opp바이어스(⑥e)+early(⑥c/d)+diffuse(⑥e-1)+early predelay/EQ(⑥e-3b) 라이브 + cluster 코어(⑥e-2)+EQ비콰드 코어(⑥e-3a)** + 회귀 0 + WERROR=ON **양 빌드 132/132**. push됨 **main +37**.
- **다음 증분(순서)**: ⑥ 룸 마감 잔여 — **⑥e-2-wire cluster 결선**(send :414-419 `clusterBusInput += xdel*cSend(roomClusterSend01·0.48)` + process :595-647 버스EQ(RoomBiquad HP120/LP10000=cEcHp/cEcLp)+RoomCluster::process+per-spk diffuseGains(opp-biased clusterU=normalized{opp.x·0.93,opp.y·0.93+0.35·0.07,opp.z·0.93}); xdel(⑥e-3b)+opp(⑥e 완료)+roomClusterSend01 default=0.4/diff=0.48/vol=630 의존, OSC=⑥e-4) / **⑥e-4 룸 OSC params** ⚠ **외부 API 설계 결정 필요**(레퍼런스 `/room/preset/recall` 프리셋만—per-param OSC 미러 없음 → mmhoa 고유 주소체계 설계: roomEngineEnabled gate + T60/halfExtents/roomEarly*/roomCluster*/EQ 코너. cluster-wire/튜닝 unblock. 사용자 확인 권장). → ⑦ 디코릴 ⑧ 헤드트래킹/바이노럴 ⑨ ADM ⑩ per-obj EQ.
