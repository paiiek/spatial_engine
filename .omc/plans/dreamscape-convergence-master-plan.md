...
- **현재 안정 지점**: VAP+VBAP3D+MDAP+WFS + **128 리프트** + **룸: 후기 FDN(⑥a/b) + early reflections(⑥c/d) 라이브** + 회귀 0 + WERROR=ON **양 빌드 130/130**. 브랜치 push됨 **main +28**.
- **다음 증분(순서)**: **⑥e 룸 마감** — early predelay(:374-395)+absorption EQ(:397-406)+cluster(:414-419,595-647)+uniform-diffuse 블렌드(:113-165)+late opp 소스바이어스(:543-583)+룸 파라미터 OSC plumbing(roomEarly*/roomCluster*/roomT60/halfExtents/roomEngineEnabled) → ⑦ 디코릴 ⑧ 헤드트래킹/바이노럴 ⑨ ADM ⑩ per-obj EQ. (⑥e 더 쪼갤 수 있음: ⑥e-1 diffuse+opp, ⑥e-2 cluster, ⑥e-3 predelay+EQ, ⑥e-4 OSC).
