...
- **▶ 현재 위치 / 다음 (resume 포인터)**: Phase 0 ✅ · Phase 1(성능) ✅ · Phase 3(ADM 코어) ✅ · **Phase 2(바이노럴) ✅** · **Phase 4.1(per-obj DSP /obj/dsp) ✅ 검증** · **Phase 5 디리스킹(soak xrun=0 + 머지 클린) ✅** → **다음 = Phase 4.2 (캘리브: 가짜 pink 교체+Voss-McCartney 7-state+log-sweep 20→20k+speaker-check) → 4.3 (50슬롯 `/layout/slot`)** → Phase 5 잔여(머지 실행 +9커밋 흡수, v1.0-rc 태그, flake 제거). ⚠ 룸 O(spk²) 최적화=v1.1. ⚠ B2 헤드트래킹(SH 회전)=v1.1. **Phase 번호는 이 v1.0 플랜이 정준**.
