...
- **▶ 현재 위치 / 다음 (resume 포인터)**: Phase 0 ✅ · Phase 1(성능) ✅(1.5 skip 정당화) · Phase 3(ADM 코어) ✅ · **Phase 2 설계 ✅ → 다음 = 증분 2.6a 헤드회전 코어**(`Coords.h::rotate_engine_dir_by_head` 순수함수 + 골든 `test_convergence_head_rotate`; yaw+30°+정면→az'=+30°=R-louder; 오디오/OSC/세션 무접촉; 양빌드 회귀0). 그 후 2.6b(/ypr+wire) → 2.1/2.4/2.5 → Phase 4 → Phase 5(릴리스: 머지 FF + 30분 soak RT-우선 재실행). ⚠ 룸 O(spk²) 최적화=v1.1. **Phase 번호는 이 v1.0 플랜이 정준**(구 `dreamscape-convergence-master-plan.md`는 SUPERSEDED, 번호 다름).
