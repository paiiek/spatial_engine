...
- **▶ 현재 위치 / 다음 (resume 포인터)**: Phase 0 ✅ · Phase 1(성능) ✅(1.5 skip 정당화) · Phase 3(ADM 코어) ✅ · Phase 2 설계 ✅ · **2.6a 헤드회전 코어 ✅ → 다음 = 증분 2.6b (/ypr OSC + 세션 head 멤버 + B1 wiring)**: 신규 `CommandTag::SysHeadYpr=0x1C`/`PayloadSysHeadYpr{yaw,pitch,roll}`, `/ypr ,fff`(+alias `/sys/ypr`) 디코드(aed 패턴 복제), 엔진 atomic head 멤버×3(relaxed), `SpatialEngine.cpp:1718` setDirection **직전** `rotate_engine_dir_by_head` 호출, 실엔진 in-proc 골든 + `smoke_adm_az.py` 패턴 실바이너리 스모크(/ypr 30 0 0 → R-louder). B2 헤드트래킹=v1.1(SH 회전). 설계=`docs/design/PHASE2_BINAURAL_DESIGN.md` §2.6b. 그 후 2.1→2.4→2.5 → Phase 4 → Phase 5(릴리스: 머지 FF + 30분 soak RT-우선 재실행). ⚠ 룸 O(spk²) 최적화=v1.1. **Phase 번호는 이 v1.0 플랜이 정준**(구 `dreamscape-convergence-master-plan.md`는 SUPERSEDED, 번호 다름).
