...
- **▶ 현재 위치 / 다음 (resume 포인터)**: Phase 0 ✅ · 1 ✅ · 3 ✅ · 2 ✅ · 4.1 ✅검증 · **4.2(pink/sweep/passthrough) ✅** · Phase 5 디리스킹 ✅ → **다음 = Phase 4.3 (50슬롯 `/layout/slot` 런타임 레이아웃 라이브러리, SceneController 확장; ⚠ 런타임 레이아웃 스왑=glitch 리스크 주의)** → Phase 5 잔여(머지 +9커밋[클린], v1.0-rc, **flake 제거=ambi_decoder_type_swap_concurrent 5.2**). ⚠ 룸 O(spk²)=v1.1, B2 헤드트래킹=v1.1. **Phase 번호는 이 v1.0 플랜이 정준**.
