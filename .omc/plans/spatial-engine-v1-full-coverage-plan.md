...
- **다음 = Phase 1.3 DBAP/VAP no-alloc**: VBAP의 `_into`+per-block 캐시 패턴 이식 → `dbap_gain` std::vector 제거(RT 계약). VAP gain 캐시 없음도 점검. (이어 1.4 타이밍 프로브+스피커 스윕+soak → 1.5 선택 SIMD.) ⚠ Phase 1.2 발견: dbapForPosition이 `AlgorithmAnalyticReference::dbap_gain`(std::vector 반환) 호출 = 활성 시 매블록 RT 할당 → 1.3 핵심 타깃.
