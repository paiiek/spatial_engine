...
- **✅ Phase 2 증분 2.4 delay ring 완료 (2026-06-07, code-reviewer APPROVE)** — 최종 바이노럴 L/R 버스의 65536-tap stereo 딜레이 링(레퍼 BinauralMonitorChain.cpp:132-154 byte-faithful). 적용점 = render_branch 직후·finalizeXfadeBlock 후·**EQ 앞**(레퍼 순서 HRTF→delay→EQ→limit). `ds=(int)(ms·0.001·sr+0.5)` clamp[0, cap−2], ds==0=passthrough(쓰기 인덱스는 전진, 레퍼 동일). 링 `std::vector` = prepare서 할당·0클리어(컨트롤스레드), 오디오 경로는 modulo 인덱싱만(alloc 0); 쓰기 인덱스 `bin_delay_write_` 오디오스레드 전용. **read-before-write**(같은 iteration서 덮어쓸 샘플을 먼저 읽음 — tap=block 경계 무관). mmhoa superset: tap 튜너블 `/sys/binaural_delay ,f ms`(`CommandTag::SysBinauralDelay=0x1F`+payload+variant, control-thread atomic store early-return=/ypr 패턴, 블록당 1회 read). 기본 0ms=무지연. 테스트 `test_convergence_binaural_delay`(① OSC decode roundtrip ② **풀엔진 골든**: obj0 온셋 envelope, tap0 vs tap50ms → onset shift ≈2400샘플(plateau-bias 회피 위해 envelope xcorr 대신 per-run onset 검출), 총에너지 보존). 스모크 `smoke_binaural_delay.py`(실바이너리: tap200ms → onset +9856샘플≈205ms, xruns=0). **양 빌드 149/149 green WERROR+RT_ASSERTS**(148→149, 회귀0).
- **✅ Phase 2 (바이노럴) DSP 체인 실질 완료**: 2.6a/2.6b(헤드트래킹) + 2.5(5밴드EQ) + 2.1(prefeed LP) + 2.4(delay ring) 전부 구현·검증·커밋. **2.2(FIR 에너지 랭킹) = Option A에서 미구현(설계 근거: B1=O(활성객체) bounded, B2=고정24VS, 128→24 랭킹 unmotivated; speaker-bus 모니터 C 도입 시에만 필요)**. **2.3(HRTF 합성)=기존 자산 사용**(B1 KdTree + B2 t-design VS, 신규 코드 0). 잔여 비DSP: 세션 직렬화에 binaural EQ/delay/prefeed/head 멤버 영속(scene save/load)=선택(현재 OSC 런타임 제어로 충분, 기본값 하위호환). ⚠ 비차단 후속: B2 헤드트래킹(SH Wigner-D 회전)=v1.1.
- **✅ Phase 4.1 검증 완료 (2026-06-08, d2387f7, 푸시됨)** — `/obj/dsp` 라우팅은 **이미 결선돼 있었음**(플랜의 "완성"은 stale): decode(CommandDecoder.cpp:754)→FIFO→drain(SpatialEngine.cpp:1234)→obj_cache_→PerObjectChain.setParams(1367)/processSample(1380), param 0–3 EQ dB·4 delay·5 k_hf·6 reverb_send·**7 width**(ObjectState→VBAP/DBAP MDAP) 전부 동작. 실바이너리 스모크 `scripts/smoke_obj_dsp.py` 신규: per-obj EQ(-18dB@band0 → obj0 110Hz 55× 컷, obj7 495Hz 유지=객체격리) + width(param7 → MDAP active_ch 3→4, L1 0.65, 골든 test_convergence_mdap과 일치), xruns=0. **핵심 교훈**: `/obj/dsp`는 **seq/id 엔벨로프 필수** `,iiiif seq id obj param value`(CommandDecoder.cpp:303-314, smoke_dbap의 /obj/algo와 동일 관례) — bare `,iif`는 obj/param이 seq/id로 먹혀 width가 EQ band0로 오라우팅(무증상). **엔진 무결, 코드변경 0**. 4.1은 사실상 done(검증으로 확정), 4.2 캘리브(가짜 pink 교체+Voss-McCartney+log-sweep)/4.3 50슬롯이 Phase 4 잔여 실작업.
- **✅ Phase 5 디리스킹 부분 완료 (2026-06-08)** — ① **30분 soak xrun=0 재현 ✅**(조용한 호스트, plain 스케줄링: 12spk/8obj 1800s, blocks=337878, **xruns=0**, p99_max=3055µs<3733µs, cpu_peak 85%). **이전 xrun=1은 빌드+리뷰어 경합 아티팩트로 확정 → DoD④ 충족**(RT-우선 chrt -f는 sudo 필요라 미적용이나 plain서 이미 0). ② **머지싱크 점검 ✅**: ⚠ plan §6 "main frozen" 가정 **무효**(main이 9커밋 전진=ADR0019 PR6 soak 하니스+문서+IPC `sendReply(,iis)` 수정). convergence 88-ahead/9-behind. **충돌표면 단 1파일** `core/src/ipc/OSCBackend.cpp`, **`git merge-tree` exit 0 = 자동머지 클린**(수동 충돌해결 불필요). 머지=Phase 5에서 9커밋 흡수만 하면 됨.
- **✅ Phase 4.2a 실(實) pink noise 완료 (2026-06-08, 6859071, 푸시됨, code-reviewer APPROVE)** — 캘리브 노이즈의 가짜 pink(1-pole LP ~−6dB/oct) → **진짜 −3 dB/oct Kellet 7-state**. 신규 `core/src/dsp/PinkNoise.h`(`spe::dsp::PinkKellet`, 헤더온리, RT-safe 7 inline float, 무할당). ⚠ **레퍼런스 비-바이트충실**(레퍼 gains 0.822…=측정 −5.3dB/oct=가짜 pink → 표준 canonical Kellet gains 채택, 프로젝트 self-tuned-DSP 룰 + 헤더에 근거/provenance 문서화). NoiseChan가 필터 by-value 보유, audioBlock서 processSample, drain이 white→pink 전이 시 reset(오디오스레드, 무경합). 단위 `test_convergence_pink_noise`(해석적 전달함수 slope=**−3.010 dB/oct**, RNG/FFT 없는 결정론 + canonical-coeff + reset). 스모크 `smoke_noise_calib.py`(라이브 pink −3.01 vs white +0.04, xruns=0). **양 빌드 150/150**(149→150), 회귀0. ⚠ 빌드교훈: 헤더 변경 후 **엔진 바이너리 명시 재빌드 필요**(test 타깃만 빌드하면 스모크가 stale 엔진 실행).
- **✅ Phase 4.2b log-sweep 완료 (2026-06-08, code-reviewer APPROVE)** — `/noise/{ch}/type`에 **sweep** 추가(white|pink|sweep). 신규 `core/src/dsp/LogSweep.h`(`spe::dsp::LogSweep`, 20→20k 1s exp, 레퍼 AudioEngine.cpp:985-996 byte-faithful: pos+=1/sr wrap1.0, instFreq=f0·(f1/f0)^pos, phase적분, sin; 헤더온리 RT-safe 1float+1double). **bool pink→uint8_t mode(0/1/2) 전(全)경로 리팩터**(Command.h/CommandFifo.h/decode+echo/dispatch/drain/audio loop/accessor, noisePinkAt=mode==1 compat shim 유지, 9사이트 일관). NoiseChan에 sweep_gen 추가(pink_filt 유지). drain이 mode 변경 시 해당 generator reset(오디오스레드). 단위 `test_convergence_log_sweep`(instFreq law 20/632.46/20000 = 지수법칙 증명 + 출력[-1,1] + zero-crossing 상승 0→361 + reset). 스모크 `smoke_sweep_calib.py`(라이브: **crest factor 1.41**=tonal sweep≠noise, local freq 0..18300Hz span, xruns=0). 테스트 2개 mode 필드 업데이트. **양 빌드 151/151**(150→151, build-128서 `ambi_decoder_type_swap_concurrent` #85 1회 flake→rerun PASS=Phase5.2 기지 flake, 무관). code-reviewer APPROVE(0 CRITICAL/HIGH, stale 코멘트 2개 수정).
- **✅ Phase 4.2c speaker-check 입력 passthrough 완료 (2026-06-08, code-reviewer APPROVE)** — `/noise/{ch}/type passthrough`(mode 3) = 하드웨어 입력채널→스피커 라우팅(레퍼 checkMode 3, 배선 확인용) + 신규 `/noise/{ch}/source ,i <in>`(`CommandTag::NoiseSource=0x42`+`PayloadNoiseSource`). bool→mode는 4.2b서 완료라 mode 3만 추가. audio loop: in_ptr를 **블록당 1회** 해소(`input_channels[in_src]` iff non-null & 0≤src<count, else nullptr→무음), 샘플루프선 `in_ptr?in_ptr[n]:0`(OOB/null 완전가드). NoiseSource 플러밍 9사이트 NoiseGain 미러, `/source ,i`=단일선두int(seq/id 트랩 무관). 단위 `test_convergence_speaker_check`(**실 audioBlock + 합성 입력블록**: input[2]→spk1 라우팅·source repoint·null입력→무음·OOB src→무음·mode3→0→3 전이+in_src 영속). decode 테스트에 passthrough+NoiseSource 라운드트립 추가. **양 빌드 152/152**(151→152). ⚠ 실바이너리 UDP 스모크는 헤드리스 입력주입(shm) 필요라 생략 — in-proc 통합테스트가 실 오디오경로 커버(reviewer 승인). **Phase 4.2(캘리브) = pink+sweep+passthrough 전부 완료.**
- **📐 Phase 4.3 아키텍처 결정(architect, 2026-06-08)**: 런타임 레이아웃 스왑 = **refined Option B = quiesce + in-place re-prepare**. ⚠ **핵심**: `render_ready_` 게이트만으론 불충분 — audioBlock 최상단 OSC 드레인이 `layout_`/`noise_chans_`/`spk_limiters_`를 **언게이트로 읽음**(SpatialEngine.cpp:1142-1250). 따라서 **quiescence handshake**(AmbiDecoder/SOFA-swap 관용구 재사용)로 콜백 전체 retire 후 control-thread서 in-place re-prepare. 풀 더블버퍼(Option A)=렌더러 전체+2nd BinauralMonitor를 단일 atomic 뒤로 숨겨야 함=거대 리팩터라 기각. 저장=파일기반 YAML 슬롯(LayoutLoader 재사용+save 추가). OSC=단일선두int verbs `/layout/slot/{save ,is|load ,i|clear ,i|list|current}` 전부 control-thread. ⚠ **하드 제약**: 물리 출력채널수는 백엔드 시작시 고정 → 그 이상 스피커는 silently unrouted(Inc 5 가드). **증분: Inc1 serializer → Inc2 슬롯+OSC(load=pending stage, no live apply) → Inc3 reprepareForLayout 리팩터(byte-identical) → Inc4 라이브스왑(유일 RT변경, 단독+concurrency test) → Inc5 채널수 가드**. 상세=architect 분석(이 세션).
- **✅ Phase 4.3 Inc 1 serializer 완료 (2026-06-08, bc3f520, 푸시됨)** — `geometry::save_layout(layout, path)`(LayoutLoader는 읽기만 있었음). Cartesian x/y/z를 max_digits10 정밀도로 직렬화 → `load(save(L))` 스피커 비트정확 라운드트립(az/el은 load시 lossy라 미기록). control-thread 전용, 오디오경로 무접촉. 단위 `test_convergence_layout_save`(delay/gain+비연속 채널 비트정확 라운드트립, channel→idx 보존, unwritable 경로→graceful false). 양 빌드 153/153. **리뷰: 순수 25줄 직렬화+라운드트립테스트=경량, reviewer 생략 합리적(Inc2-4 risky 증분에 리뷰 집중).**
- **▶ 현재 위치 / 다음 (resume 포인터)**: Phase 0 ✅ · 1 ✅ · 3 ✅ · 2 ✅ · 4.1 ✅검증 · 4.2(pink/sweep/passthrough) ✅ · Phase 5 디리스킹 ✅ · **4.3 Inc1(serializer) ✅** → **다음 = 4.3 Inc 2 (슬롯 라이브러리 디렉토리 + `/layout/slot/{save,clear,list,current}` OSC, load는 pending stage만; 라이브 apply 없음=오디오스레드 무접촉)** → Inc3 리팩터 → ⚠ Inc4 라이브스왑(checkpoint 권장) → Inc5 가드 → Phase 5. ⚠ 룸 O(spk²)=v1.1, B2 헤드트래킹=v1.1. **Phase 번호는 이 v1.0 플랜이 정준**.
