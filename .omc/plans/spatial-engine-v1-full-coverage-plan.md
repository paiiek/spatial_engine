...
- **다음 = Phase 1.4 단계별 타이밍 + 스피커 스윕 + soak** (Phase 1 마무리, DoD 실측): render/room/decorr/binaural 구간 프로브(steady_clock, ObservabilityCounters 패턴) → `/sys/metrics` 확장. perf harness 8→128 스피커 스윕 → `docs/RT_BUDGET_SPEAKERS.md`. 게이트 예산을 live block size 파생으로(하드코딩 933µs 제거). DoD: ①32obj/8spk+룸+바이노럴 ≤40% ②128spk 봉투 측정·문서화 ③denormal stall 제거(peak 안정) ④룸+디코릴 30분 soak xrun=0. (이어 1.5 선택 SIMD.) ⚠ perf harness 위치/현 측정 단위 먼저 조사 필요.
