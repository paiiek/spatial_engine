...
- **▶ 현재 위치 / 다음 (resume 포인터)**: Phase 0 ✅ · Phase 1(성능) ✅ · Phase 3(ADM 코어) ✅ · **Phase 2(바이노럴) ✅**(2.6a·2.6b·2.5·2.1·2.4 구현; 2.2 생략·2.3 기존) → **다음 = Phase 4 (per-obj DSP `/obj/dsp` 라우팅 완성 + 캘리브 + 50슬롯 레이아웃)** → Phase 5(릴리스: 머지 FF + 30분 soak RT-우선 재실행). 설계=`docs/design/PHASE2_BINAURAL_DESIGN.md`(Phase 2 완결). ⚠ 룸 O(spk²) 최적화=v1.1. ⚠ B2 헤드트래킹(SH 회전)=v1.1. **Phase 번호는 이 v1.0 플랜이 정준**(구 `dreamscape-convergence-master-plan.md`는 SUPERSEDED).
