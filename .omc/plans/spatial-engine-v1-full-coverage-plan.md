...
- **다음 = Phase 1.4 잔여**: (1.4b) 단계별 타이밍 프로브(render/room/decorr/binaural, steady_clock+ObservabilityCounters) → `/sys/metrics` 확장. (1.4c) DoD ④ 룸+디코릴 **실용 스피커수(≤16)** 30분 soak xrun=0 (run_soak.py 재사용, background). 그 후 1.5 선택 SIMD. ⚠ 룸 O(spk²) 최적화는 Phase 1 후속 또는 v1.1 별도 증분(대형 어레이 룸 필요 시).
