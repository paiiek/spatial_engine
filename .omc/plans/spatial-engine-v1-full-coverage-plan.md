...
- **▶ 현재 위치 / 다음 (resume 포인터)**: Phase 0 ✅ · Phase 1(성능) ✅(1.5 skip 정당화) · Phase 3(ADM 코어) ✅ · Phase 2 설계 ✅ · **2.6a ✅ · 2.6b ✅ · 2.5 5밴드EQ ✅ → 다음 = 증분 2.1 (prefeed LP 4200Hz, HRTF 입력 one-pole)**. 그 후 2.4(delay ring) → (2.2 FIR랭킹 생략·문서화, 2.3 HRTF 기존) → Phase 4(per-obj DSP+캘리브+50슬롯) → Phase 5(릴리스: 머지 FF + 30분 soak RT-우선 재실행). 설계=`docs/design/PHASE2_BINAURAL_DESIGN.md`(잔여 §2.1/2.4). ⚠ 룸 O(spk²) 최적화=v1.1. ⚠ 비차단 후속: B2 헤드트래킹(SH 회전)=v1.1. **Phase 번호는 이 v1.0 플랜이 정준**(구 `dreamscape-convergence-master-plan.md`는 SUPERSEDED, 번호 다름).
