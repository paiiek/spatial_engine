...
### SPE_DEBUG_ENDPOINTS — soak 진단 전용, 운영 노출 금지

`SPE_DEBUG_ENDPOINTS=1` 환경 변수를 켜면 `/api/_debug/asyncio_tasks` 가 활성화되어 uvicorn 이벤트 루프의 task 수와 활성 WS 커넥션 수를 그대로 노출한다 (`ui/webgui/server.py`).  
이 엔드포인트는 48h soak harness 의 G7 게이트 (`asyncio_slope_tasks_per_h ≤ 1`) 측정 전용이며, 다음 이유로 **공연·프로덕션 환경에서는 절대 켜지 않는다**:

- 내부 상태 누설 (task 수, WS 핸들 수) → 부하 추정/타이밍 공격 보조 정보가 됨
- 인증 없이 응답 — 8000 포트가 외부 노출되면 익명 접근 가능
- 운영 부하 — 1 Hz 호출이라도 `asyncio.all_tasks()` 는 잠금 비용이 있음

**권장 운용:**

```bash
# soak harness 실행 (격리된 측정 호스트, 외부 접근 차단 상태)
export SPE_DEBUG_ENDPOINTS=1
python3 tests/soak_harness/run_soak_webgui.py --duration 172800

# 측정 종료 후 즉시 해제
unset SPE_DEBUG_ENDPOINTS
```

`SPE_DEBUG_ENDPOINTS` 미설정 또는 `"1"` 외 값일 때 엔드포인트는 404 를 반환한다 (등록되지 않은 라우트와 구분되지 않음).
