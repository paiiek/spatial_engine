...
| **HRTF Binaural** | KEMAR SOFA, OLA 컨볼브, KD-tree NN lookup, B1/B2 모드 (B2=고품질 / B1=러닝 fallback), demote 디멀티플렉서 + 사용자 reset | `core/src/hrtf/HrtfLookup.cpp`, `OlaConvolver.cpp`, `KdTree3D.cpp` | `test_p_binaural*` (11), P3.4 interp oracle |
...
| **IR Convolver stub** | 멀티채널 IR validation + RT-safe FFT (현재 stub) | `core/src/reverb/IRConvolutionStub.cpp` | `test_p7_ir_metadata*` |
...
| **객체 수** | MAX_OBJECTS=64 | ~96 객체 + 멀티존 | ~128 객체 | ~118 객체 (베드+오디오 오브젝트) | 무제한 (CPU 한도) | 16 객체 |
...
| **HRTF Binaural** | ✅ KEMAR SOFA, B1/B2 demote 시스템 | ✅ (오프라인 변환) | ✅ SOFA, 7+ HRTF 데이터셋 | ✅ Dolby 자체 HRTF | ✅ SOFA, IRCAM Listen | ✅ 자체 HRTF + SOFA |
...
| **HRTF Binaural** | ✅ KEMAR + SOFA | ✅ SOFA + magLS / MagLS | ✅ SOFA | ✅ 자체 HRTF | ✅ | ✅ |
...
2. **MAX_OBJECTS=64 하드 캡** — L-ISA(~96) / Atmos(118) / Spat Pro(무제한) 대비 적음. PR-track 으로 확장 가능하나 RT-budget 검증 필요.
3. **고차 Ambisonic (5–7차) 미지원** — IEM/SPARTA 는 7차까지. M2HOA-Q10 open.
4. **GUI 가 PySide6/WebGUI** — 상용 L-ISA Controller 의 워크플로우 (씬 라이브러리, snapshot 자동화) 와 비교하면 미숙.
...
### 5.2 v0.9 (다음 마이너 — 1–2개월)

| 항목 | 비고 |
|---|---|
| **ADR 0019 PR6** — 60s cross-process shm soak | shm IPC 트랙 종결 |
| **ADR 0019 PR7** — release notes + v0.8.0 태그 | |
| **WAV 파일 직접 입력 backend** (`--input-backend file:<wav>`) | adm_player 없이 단일 프로세스 테스트; `core/src/input/FileInput.cpp` 코드 이미 있음, CLI 만 연결 |
| **HRTF dataset 다양화** (CIPIC, RIEC, HUTUBS, SADIE-II) | SOFA loader 일반화 후 |
| **MAX_OBJECTS 64 → 128** | RT budget 재검증 + 메모리 풋프린트 측정 후 |
| **`SpatialEngine` god-object refactor** (P7.1) | supervised sprint, BinauralTelemetry facade 추출 |
| **vid2spatial_v3 → engine 통합** | 새 비전 모델 결과 받기 |
| **EQ / Limiter / DRC** | 출력 체인 마무리 |
| **Real-time metrics dashboard** (WebGUI 새 탭) | telemetry 채널 시각화 |
