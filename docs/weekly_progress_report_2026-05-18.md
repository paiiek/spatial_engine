...
| **P1-8 (D-M2)** | `steady_clock` vDSO availability probe + fallback | ⏳ pending | 비-x86_64-Linux platform (old ARM Linux, 일부 macOS) 에서 `clock_gettime` 가 syscall 로 떨어지면 RT-unsafe. startup probe → `getauxval(AT_SYSINFO_EHDR)` + dlsym `__vdso_clock_gettime` non-null 확인. Unavailable 시 wall-clock 측정 skip + `/sys/binaural_warning ,s "rt_timing_unavailable"` 1회 통보. |
| **P1-9 (D-M3)** | CH7 §7.5.4 신설 — "save during demote → reload re-attempts B2" 한 단락 | ⏳ pending | Architect 가 §B.1 에서 잡은 documentation gap. |
| **P1-10 (D-M4)** | Heartbeat 주기 1000 ms → 100 ms (또는 sub-tick 스케줄러) | ⏳ pending | Host handshake 지연 시 latch 200 ms latency 예산을 보장. status 는 1 Hz 유지, latch drain 만 가속. |
| **P1-11 (MEDIUM-2)** | release notes / changelog 의 "Release validation" 섹션 보강 | ⏳ pending | ARM/macOS/DAW host 0 검증 명시 (현재 #9 절 안에 묻혀있음 — prominent 화). |
| **P1-12 (MEDIUM-3)** | ADR 0016 Appendix A — GPL §6 obligation mapping per band + 법률 review status disclose | ⏳ pending | Critic §A.3 — Band 1 handoff 전 외부 법률 review 권장 명시 + "written offer for 3 years" 옵션 표 |
| **P1-13 (MEDIUM-1)** | P1-3 integration ctest rename 또는 한 줄 disclaimer | ⏳ pending | "What this test EXPLICITLY DOES NOT cover:" 절 추가, 또는 `test_b2_runtime_demote_forwarders.cpp` 로 rename. |
