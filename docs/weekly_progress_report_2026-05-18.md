...
### §5.0 P-tag 우선순위 체계 (정의)

본 리포트 + ADR 0016 + release notes + macOS verify 체크리스트가 cross-reference 하는 P-tag 정의:

| Tier | 의미 | 처리 시점 |
|------|------|-----------|
| **P0** | Blocker — 외부 사용자 인계 전 반드시 | 다음 commit 안 |
| **P1** | Should-fix in v0.6.x | v0.6.x patch 사이클 |
| **P2** | v0.7 candidate | v0.7 release cycle |
| **P3** | Long-term | v1.0+ 또는 외부 자원 필요 |

각 항목에 ✅ (closed) / 🔄 (in progress) / ⏳ (pending) 상태 표시. status 가 변하면 본 표 즉시 업데이트.

### §5.1 P0 — Blocker (외부 사용자 인계 전 반드시)

| Tag | 항목 | 상태 | 비고 |
|-----|------|------|------|
| **P0-1** | DAW (Reaper / Bitwig) 16-체크박스 실측 + screenshot 2장 + WAV 2개 | ⏳ user-handson | self-claim 검증의 유일한 falsifier. ADR 0016 §Band-1 ritual 적용 (≤5 테스터, 서면 ack). `docs/release/v0.3.0/daw-handson-log.md` 양식 사용. |
| **P0-2** | License / 배포 정책 ADR — JUCE GPL-3 viral trigger 명문화 | ✅ closed | `docs/adr/0016-external-distribution-policy.md` (Band 0~3 분류, ≤5 베타 cap). 단, MEDIUM-3 (Appendix A GPL §6 mapping + 법률 review status disclose) 가 v0.6.x 안 follow-up. |
| **P0-3** | CHANGELOG + release notes 5건 (v0.3.1 ~ v0.6) | ✅ closed | `CHANGELOG.md` + `docs/release/v0.3.1`/`v0.4.0`/`v0.5.0`/`v0.5.1`/`v0.6.0`. GHA `release.yml body_path` 정상화. |
| **P0-4** | 실 24h WebGUI soak 1회 + 결과 commit | 🔄 in progress | `soak_reports/soak_webgui_day1_20260518_103113.json` 진행 중 (PID 89276, 2026-05-19 10:31 종료 예상). 결과 sentinel 분석 + commit 은 종료 후. |

### §5.2 P1 — Should-fix in v0.6.x

| Tag | 항목 | 상태 | 비고 |
|-----|------|------|------|
| **P1-1** | 바이노럴 한국어 매뉴얼 챕터 | ✅ closed | `docs/manual_kr/CH7_BINAURAL.md` (~750 lines, OSC outbound 채널 + B1/B2 선택 + troubleshooting + SOFA 가이드). MEDIUM-1 (CH7 §7.5.4 의 demote re-eval on reload 한 줄) 추가 follow-up. |
| **P1-2** | v0.6 retroactive ralplan (architect-r + critic-r) | ✅ closed | `.omc/plans/architect-r-v0.6-retro.md` + `critic-r-v0.6-retro.md`. APPROVE WITH RECOMMENDATIONS / ACCEPT-WITH-RESERVATIONS. 본 doc 의 D-M*, MEDIUM-*, LOW-* 항목은 그 review 산출물. |
| **P1-3** | runtime_underrun 통합 ctest (audio_block 끝-끝) | ✅ closed | `test_b2_runtime_underrun_engine_integration.cpp` (ctest 85→86). MEDIUM-1 follow-up: test 한계 self-disclose 한 줄 또는 rename. |
| **P1-4** | B2 timing wrapper kill-switch (sticky 후 skip) | ✅ closed | `SpatialEngine.cpp:723-741` 의 wall-clock 브래킷을 `isRuntimeDemoted()` guard. |
| **P1-5 (D-M1)** | `BinauralMonitor::initialize()` 가 runtime_demote_* 3 atomic reset | ✅ closed | Architect retroactive review 가 silent bug 발견 — "sticky until next prepareToPlay" 계약이 사실 미reset. `BinauralMonitor.cpp:33-50` fix + `test_b2_runtime_underrun_auto_demote.cpp` 의 `initializeResetsDemoteScenario` 회귀 게이트. |
| **P1-6 (HIGH-1)** | `docs/ipc_schema.md` 에 v0.5.1+ outbound 채널 명시 (`/sys/binaural_status`, `/sys/binaural_warning`, `/sys/state ,s "fallback_mode=..."`) | ✅ closed | Critic 가 발견 — 사용자 매뉴얼이 ipc_schema.md 를 canonical 로 cite 하는데 v0.5.1+ 채널 0 회 등재. `docs/ipc_schema.md:§Binaural Telemetry` 절 신설 + `/sys/state` dual-tag (additive) 명시. |
| **P1-7 (HIGH-2)** | weekly_progress_report §5 의 P-tag 명시 + cross-reference 정합성 | ✅ closed | 본 리비전. ADR 0016 / RELEASE_NOTES / CH7 / macOS verify 의 5+ cross-reference 들이 §5 본문에서 검색 불가하던 문제 해결. |
| **P1-8 (D-M2)** | `steady_clock` vDSO availability probe + fallback | ✅ closed | `BinauralMonitor::probeSteadyClockSpeed()` 가 10000-sample 타이밍 (`avg ≥ 200ns/call → slow`) 으로 platform 판정. slow 시 (a) `SpatialEngine.cpp` 의 wall-clock 브래킷이 `binaural_.isSteadyClockFast()` 게이트로 skip, (b) heartbeat 가 `/sys/binaural_warning ,s "rt_timing_unavailable"` 1회 통보. `steadyClockSlowPathScenario` 회귀 게이트 추가. |
| **P1-9 (D-M3)** | CH7 §7.5.5 "save during demote → reload re-attempts B2" 단락 | ✅ closed | Architect 가 §B.1 에서 잡은 documentation gap. land. |
| **P1-10 (D-M4)** | Heartbeat 1000 ms → 100 ms + sub-tick scheduler | ✅ closed | `kTicksPerStatusEmit = 10` 으로 `/sys/binaural_status` 는 1Hz 유지, latches 는 10Hz 드레인. 최악 latency 1000 ms → 100 ms. |
| **P1-11 (MEDIUM-2)** | release notes / changelog "Release validation" 섹션 보강 | ✅ closed | `### Coverage gaps (intentional, surfaced for honesty)` 신설 — ARM/macOS/DAW host 0 검증 명시 + "Treat any v0.6 claim of 'DAW-ready' as conditional on these gates being closed". |
| **P1-12 (MEDIUM-3)** | ADR 0016 Appendix A — GPL §6 obligation mapping per band + 법률 review status disclose | ✅ closed | Appendix A 신설 (4 bands × §6 election + per-band audit-log 요구사항) + `## Limitations & legal review status` (외부 법률 review 권장 명시). |
| **P1-13 (MEDIUM-1)** | P1-3 integration ctest rename 또는 한 줄 disclaimer | ✅ closed | `"What this test EXPLICITLY DOES NOT cover"` 절 + `"this is a *forwarder + sticky-semantics integration* test, not a full end-to-end audio path test"` 명시. |
| **P1-14 (MEDIUM-4)** | §6.1 "동일 파이프라인" 표현 정정 | ✅ closed | 본 리비전 — `§6.1` 마지막 줄 명시적으로 "v0.6 는 post-hoc cadence; retroactive 로 land" 로 정정. |

### §5.3 P2 — v0.7 candidate

| Tag | 항목 | 상태 | 비고 |
|-----|------|------|------|
| **P2-1** | GHA arm64 + macOS matrix | ✅ structural (signal-only) | `.github/workflows/cross-platform.yml` — `continue-on-error: true` 로 v0.6 cycle 동안은 signal-only. 첫 stable green 후 required 로 promote. |
| **P2-2** | 외부 베타 테스터 모집 + onboarding 키트 | ⏳ pending | 5-10명 (한국 작곡/음향 커뮤니티). P0-1 PASS 후 진행. ADR 0016 §Band-1 5-cap 적용. |
| **P2-3** | B2 probe accuracy 보정 — #5 sticky-demote 데이터 기반 startup threshold 조정 | ⏳ pending | 닭-달걀: #5 telemetry 수집 후에야 가능. D-S3 (`/sys/binaural_diag ,iif` 채널 신설) 가 enabler. |
| **P2-4 (D-S1)** | OSC `/sys/binaural_reset_demote ,i 1` — 사용자 명시적 reset hatch | ⏳ pending | 60초 cooldown 으로 ping-pong 방지. 베타 피드백 unlock. |
| **P2-5 (D-S2)** | Block-size-aware strike count — `max(8, ceil(0.02s / block_seconds))` | ⏳ pending | 32-sample DAW + 1024-sample render host 양극단에서의 false-positive/late-detection 균형. |
| **P2-6 (D-S3)** | `/sys/binaural_diag ,iif <block_size> <sample_rate> <max_ratio>` telemetry 채널 | ⏳ pending | 신규 multi-arg sendReply overload 필요 (architect §A.3). soak harness 가 jsonl 로 로깅. |
| **P2-7 (D-S4)** | Relacy race-detector dev-dep + `test_osc_outbound_relacy` | ⏳ pending | #9 release-store 의 weak-memory-order 정확성을 ARM 하드웨어 없이 synthetic 검증. |

### §5.4 P3 — Long-term

| Tag | 항목 | 상태 | 비고 |
|-----|------|------|------|
| **P3-1** | MUSHRA / ABX 청취 평가 (5-10명 패널) | ⏳ pending | 외부 패널 모집 필요. binaural quality marketing claim 의 유일한 evidence. |
| **P3-2** | HUTUBS / SADIE / ARI 등 다른 SOFA 패밀리 회귀 게이트 | ⏳ pending | 현재 KEMAR 단일 검증. HRTF 개인화 / 외부 호환성. |
| **P3-3** | macOS CoreAudio backend 구현 | ⏳ pending | `docs/SETUP_MACOS.md:121` 의 한계 해소. macOS 사용자 실시간 모니터링 가능화. |
| **P3-4** | VST3 외 AU / AAX 포팅 | ⏳ pending | Logic / Pro Tools 진입. |
| **P3-5 (D-L1)** | RT-safe SPSC ring for audio-thread → IO-thread OSC messages | ⏳ pending | 본질적으로 (P3-6) head-tracking 같은 고-frequency 채널 도입 시에만 정당화 가능. |
| **P3-6 (D-L2)** | EWMA-smoothed demote detector | ⏳ pending | D-S3 telemetry 가 enabler. |
| **P3-7 (D-L3)** | 자동 re-arm of demoted state — N초 good blocks 후 B2 retry | ⏳ pending | B1→B2 reverse transition direction 검증 선행 필요 (현재 prepareToPlay 에서만 exercise). |
| **P3-8 (D-L4)** | Post-hoc plan policy 자동 강제 — `[plan-shadow]` commit tag + commit-msg hook | ⏳ pending | ralplan-required 정책의 구조적 무결성. |
| **P3-9** | Stage 2 IPC 스키마 flip | ⏳ pending | 외부 사용자 캡처 데이터 ≥ 3개 누적 시 `__schema_version__` flip. ADR 0016 Band 1+ 가 enabler. |
| **P3-10** | Decoder 추가 (AllRAD2 / ESLD 등) | ⏳ pending | 사용자 피드백 또는 학술적 비교 필요. |
| **P3-11** | Lab 운영용 standalone GUI 강화 (PySide6) | ⏳ pending | v0.4 부터 VST3 우선 — 일시 정지 중. |
...
**v0.6 는 본 4-단계 파이프라인을 통과하지 않았다 (interrupt 로 인한 post-hoc cadence).** plan doc / architect / critic 산출물이 사후 작성됨 (`spatial-engine-v0.6-stability.md`, `architect-r-v0.6-retro.md`, `critic-r-v0.6-retro.md`). 본 process gap 은 §5.1 P0~P1 의 honesty disclosure 대상이며, 다음 사이클부터 다시 fully gated 로 복귀. 누적 방지 가이드라인은 architect retro §C.1 + critic retro §A.1 참고 — 2 사이클 연속 post-hoc 시 process review task 발동.
